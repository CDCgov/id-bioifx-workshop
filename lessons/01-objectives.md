---
layout: page
title: Workshop Objectives
sidebar: workshop_sidebar
topnav: topnav
permalink: /lessons/01-objectives/
---
<!-- filepath: /Users/nbx0/repos/id-bioifx-workshop/lessons/01-objectives.md -->

## 1. Understand basic computer architecture and operating systems

- [Training Overview]({{ site.baseurl }}/lessons/02-overview-vcm/)
- [Computer Basics]({{ site.baseurl }}/lessons/03-computer-basics/)
- [Computer Setup (WSL)]({{ site.baseurl }}/lessons/04-computer-setup-wsl/)
- [Computer Environments]({{ site.baseurl }}/lessons/09-computer-environments/)
- [Computer Environments Practical]({{ site.baseurl }}/lessons/09b-comp-env-practical/)
- [Containers and Registries]({{ site.baseurl }}/lessons/10-containers-registries/)

## 2. Execute command-line operations

- [Intro to Command Line Interface]({{ site.baseurl }}/lessons/05-cli-intro/)
- [CLI Exercises]({{ site.baseurl }}/lessons/06-cli-intro-exercises/)

## 3. Inspect genome assembly

- [Sequence Alignment Overview]({{ site.baseurl }}/lessons/12-sequence-alignment/)
- [Genome Assembly and MIRA-NF]({{ site.baseurl }}/lessons/11-genome-assembly/)

## 4. Perform informatic analyses (e.g., alignment, phylogenetics, variants) of influenza sequences

- [BLAST Fundamentals]({{ site.baseurl }}/lessons/13-blast/)
- [Identifying Genetic Markers]({{ site.baseurl }}/lessons/14-identifying-genetic-markers/)
- [Phylogenetics]({{ site.baseurl }}/lessons/15-phylogenetics/)

## 5. Generate genomic surveillance reports on circulating subtypes and clades

- [Genomic Epidemiology]({{ site.baseurl }}/lessons/16-genomic-epi/)
- [Reporting with Quarto]({{ site.baseurl }}/lessons/18-reporting/)

## 6. Automate influenza genomic data processing

- [Intro to Bioinformatics Programming]({{ site.baseurl }}/lessons/07-bifx-programming/)
- [Bioinformatics Programming Exercises]({{ site.baseurl }}/lessons/08-bifx-programming-exercises/)
- [Quality Management Systems]({{ site.baseurl }}/lessons/17-quality-management-systems/)
- [Workflow Pipelines]({{ site.baseurl }}/lessons/19-workflow-pipelines/)

## 7. Submit timely, high-quality data to GISAID and NCBI

![Submit to public repositories]({{ site.baseurl }}/assets/images/mira-logo-submit-to-public.png)
